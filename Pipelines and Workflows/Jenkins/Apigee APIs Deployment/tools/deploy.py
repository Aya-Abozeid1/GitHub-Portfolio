...
def httpCall(verb, uri, headers, body):
    if httpScheme == 'https':
        conn = http.client.HTTPSConnection(httpHost, timeout=300)
    else:
        conn = http.client.HTTPConnection(httpHost, timeout=300)
    ...
    hdrs['Authorization'] = 'Basic %s' % base64.b64encode(UserPW.encode("ascii")).decode("ascii")
...
if ShouldDeploy:
    ...
    for d in deps:
        if d['environment'] == Environment and \
            ...
            print('Undeploying revision %i in same environment and path:' % \
                    d['revision'])
            previous_revision = copy.copy(d['revision'])
    ...
    if resp.status != 200 and resp.status != 201:
        ...
        ('/v1/organizations/%s/apis/%s/deployments' +
                ...
                '&revision=%i' +
                '&basepath=%s') % \
            (Organization, Name, Environment, previous_revision, BasePath),
        hdrs, None)
        print('Re-Deploy to the previous revision with status %i:\n%s' % (resp.status, resp.read()))
        sys.exit(2)
